...
def iterative_model_training(
    ...
    MODEL_TYPE = "PAGE_CATEGORY"):
      ...
      for file in file_list:
        ...
        try:
            print(f'\t $$$$$$$$$$$$$ Processing Each file:{file}')
            with mlflow.start_run(run_name=file) as run:
                ...
                train_file_name_abs = os.path.join(r"/MODELS/KPO/RE_TRAIN",Path(file).stem + f'_train_{date_time_}.json')
                val_file_name_abs = os.path.join(r"/MODELS/KPO/RE_TEST",Path(file).stem +f'_val_{date_time_}.json') 
            ...
            print(f'\t Unable to log to Traking Server ')
            print(f'Exception {str(e)}')  
            print(f'Trace back : {traceback.format_exc()}')
      return 
...
if __name__ =='__main__':
    traking_uri = "http://aisf.preludesys.com:5000"
    try:
        mlflow.set_tracking_uri(traking_uri)
        print(f'\t Connected to  Traking Server: {traking_uri}')
    except Exception as e:
        print(f'\t Unable to Connect to Traking Server ')
        print("\t",e)  
    while True:
        print(f'------------------------ Igniting Training Engine -----------------------')
        for i in range(5):
            print("\r")
            print(f'------------------------ Igniting Training Engine -----------------------')
            time.sleep(1)
        category_files = [file  for file  in os.listdir(sys.argv[1]) if file.endswith(".json") ] 
        segmenter_files = [ file for file  in os.listdir(sys.argv[1]) if file.endswith(".jsonl")] 
        print(f'Training Directory : {sys.argv[1]}')
        print(f'\t\t {os.listdir(sys.argv[1])}')
        print(f'\tcategory_files : {category_files}')
        print(f'\segmenter_files : {segmenter_files}')
        if not (category_files or  segmenter_files):
            print(f'\t ###### No Training Files found ##########')
            time.sleep(3000)
            continue
        

        print(f'\t--------- Preparing Configs--------------')
        ...
        file_dir = os.path.join(sys.argv[1]) 
        model_store_dir = os.path.join(sys.argv[2]) 
        ...
        target = sys.argv[10]
        exlcude= sys.argv[11].split(',') # if isinstance(sys.argv[11],list) else [sys.argv[11]] #['Duplicates']
        ...
        if category_files:
            print(f'\t\t$$$$$$$$$$$$$ Training Categorizer Models ##########')
        ...
        if segmenter_files:
            print(f'\t\t########## Training Segmenter Models ##########')
            target = "Isstart"
