...
model_store_dir = os.path.join(r"/KPO/MODELS")
...
def tokenize_data_with_tokenizer(tokenizer,df,features,label,max_length,model_store_dir,save_model_name):   
    print(f'\t Sample features : {df[features].head(2)}')
    print(f'\t Sample label : {df[label].head(2)}')
    encoded_data_train = tokenizer.batch_encode_plus(
        df[df.data_type=='train'][features].values, 
        add_special_tokens=True, 
        return_attention_mask=True, 
...
def initialize_model_raw(bert_sequence_config,model_instance ,
    ...
    if model_files:
      ...
      try:
        ...
        print(f'\t Exception while loading model weights ')
        print(f'\t Exception : {e} ')
        print(f'Trace back : {traceback.format_exc()}')
      print(f"loaded model weights from  file:{model_path}")
    else:
...
def initiate_training_custom(model,dataloader_train,dataloader_validation,epochs,model_store_dir,save_model_name,
                             optimizer,scheduler):
    print(f'initiate_training_custom')
